...
import { selectProductList,selectCartList } from "../redux/selector";
...
class CartScreen extends Component {
  ...
  renderPrice = () => {
    ...
    let price = 0;
    cart_list.map((cart) => {
      price = price + (cart.discounted_price * cart.count)
    })
    return price
  }
  ...
  renderBody = () => {
    return <View style={{ flexGrow: 1, flex: 1 }}>
      ...
      <Text style={{ bottom: 20, position: 'absolute', alignSelf: 'center', fontSize: FontSize(2) }}> Subtotal: ${this.renderPrice()}</Text>
